...
# For Structured Output (Dictionary)
class PassengerOutput(BaseModel):
    response : str
    isWeightExceed : bool

# Agent For Input Guardrail
...
# Agent For Output Guardrail
phd_guardrail_agent = Agent(
    name = "PHD Guardrail Agent",
    instructions="""
        You are a PHD Guardrail Agent that evaluates if text is too complex for 8th grade students. If the response is
